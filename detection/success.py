...
class LoginSuccessDetector:
    ...
    def detect(self, response, original_url: str, original_content_length: int,
        ...
        # Apply baseline normalization if baseline is available
        if baseline_result:
    ...
    def _apply_baseline_normalization(self, current_score: int, signals: List[Signal],
        ...
        """
        Normalize confidence score based on baseline login result.
        If test response matches baseline patterns (redirect, cookie, etc.), boost confidence.
        """
        ...
        # Check if current response matches baseline patterns
        ...
        # Redirect URL match (strong indicator)
        ...
        # If we have positive signals and baseline had similar patterns, boost confidence
        positive_signals = [s for s in signals if s.signal_type == SignalType.POSITIVE]
        if positive_signals and similarity_bonus > 0:
            # Additional boost if we have similar positive signals as baseline
        ...
        # Apply bonus but cap it to avoid over-scoring
